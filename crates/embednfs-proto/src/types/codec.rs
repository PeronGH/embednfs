        ...
        match self {
            ...
            NfsResop4::Open(status, res) => {
                ...
                if *status == NfsStat4::Ok
                    && let Some(res) = res
                {
                    res.stateid.encode(dst);
                    res.cinfo.encode(dst);
                    res.rflags.encode(dst);
                    res.attrset.encode(dst);
                    match &res.delegation {
                        OpenDelegation4::None => (OpenDelegationType4::None as u32).encode(dst),
                        OpenDelegation4::NoneExt(why) => {
                            (OpenDelegationType4::NoneExt as u32).encode(dst);
                            (*why as u32).encode(dst);
                            match why {
                                WhyNoDelegation4::Contention
                                | WhyNoDelegation4::ResourceNotAvail => false.encode(dst),
                                _ => {}
                            }
                        }
                        _ => (OpenDelegationType4::None as u32).encode(dst),
                    }
                }
            }
            ...
            NfsResop4::Write(status, res) => {
                ...
                if *status == NfsStat4::Ok
                    && let Some(res) = res
                {
                    res.count.encode(dst);
                    res.committed.encode(dst);
                    dst.extend_from_slice(&res.writeverf);
                }
            }
            NfsResop4::ExchangeId(status, res) => {
                    ...
                    && let Some(res) = res
                {
                    ...
                    res.sequenceid.encode(dst);
                    res.flags.encode(dst);
                    0u32.encode(dst);
                    res.server_owner.encode(dst);
                    encode_opaque(dst, &res.server_scope);
                    (res.server_impl_id.len() as u32).encode(dst);
                    for id in &res.server_impl_id {
                        id.encode(dst);
                    }
                }
            }
            NfsResop4::CreateSession(status, res) => {
                ...
                if *status == NfsStat4::Ok
                    && let Some(res) = res
                {
                    dst.extend_from_slice(&res.sessionid);
                    res.sequenceid.encode(dst);
                    res.flags.encode(dst);
                    res.fore_chan_attrs.encode(dst);
                    res.back_chan_attrs.encode(dst);
                }
            }
            ...
            NfsResop4::Sequence(status, res) => {
                    ...
                    && let Some(res) = res
                {
                    dst.extend_from_slice(&res.sessionid);
                    res.sequenceid.encode(dst);
                    res.slotid.encode(dst);
                    res.highest_slotid.encode(dst);
                    res.target_highest_slotid.encode(dst);
                    res.status_flags.encode(dst);
                }
            }
            ...
            NfsResop4::BindConnToSession(status, res) => {
                    ...
                    && let Some(res) = res
                {
                    dst.extend_from_slice(&res.sessionid);
                    res.dir.encode(dst);
                    res.use_conn_in_rdma_mode.encode(dst);
                }
            }
            ...
            NfsResop4::Lock(status, stateid, denied) => {
                    ...
                    && let Some(stateid) = stateid
                {
                    ...
                    && let Some(denied) = denied
                {
                    denied.offset.encode(dst);
                    denied.length.encode(dst);
                    denied.locktype.encode(dst);
                    denied.owner.clientid.encode(dst);
                    encode_opaque(dst, &denied.owner.owner);
                }
            }
...
fn decode_nfs_argop4(src: &mut Bytes) -> XdrResult<NfsArgop4> {
    ...
    match opnum {
        ...
        OP_BACKCHANNEL_CTL => {
            ...
            for _ in 0..count {
                ...
                match flavor {
                    ...
                    6 => {
                        let _service = u32::decode(src)?;
                        let _from_server = decode_opaque(src)?;
                        let _from_client = decode_opaque(src)?;
        ...
        OP_GET_DIR_DELEGATION => {
            ...
            let _child_attr_seconds = u64::decode(src)?;
            let _child_attr_nseconds = u32::decode(src)?;
            let _dir_attr_seconds = u64::decode(src)?;
            let _dir_attr_nseconds = u32::decode(src)?;
        ...
        OP_GETDEVICELIST => {
            ...
            let _cookieverf = decode_fixed_opaque(src, 8)?;
        ...
        OP_LAYOUTCOMMIT => {
            ...
            let _stateid = Stateid4::decode(src)?;
            let new_offset = bool::decode(src)?;
            if new_offset {
                let _last_byte = u64::decode(src)?;
            }
            let time_modify = bool::decode(src)?;
            if time_modify {
                let _time = NfsTime4::decode(src)?;
            }
            let _layout_type = u32::decode(src)?;
